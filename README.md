...
Windows COM의 특성상, 한글 프로그램이 여러 개 실행 중일 때는 하나의 인스턴스에만 연결됩니다.
다음 워크플로우를 권장합니다:
...
```
→ `list_all_hwp_windows()` 호출 → 실행 중인 모든 한글 창과 파일명 확인

#### 2번: MCP용 한글 인스턴스 생성 및 파일 열기
```
"아파트가격동향 파일을 열어줘"
```
→ `initialize_hwp()` → `open_document(파일경로)` → MCP가 제어하는 한글에서 파일 열기

#### 3번: 문서 작업 수행
```
"문서 내용을 읽어줘" 또는 "텍스트를 수정해줘"
```
→ `get_text_all()`, `find_and_replace()` 등 사용

### 예시 대화

**사용자**: "지금 열려있는 한글 파일 중에 아파트가격동향 문서 내용을 파악해줘"

**Claude 동작**:
1. `list_all_hwp_windows()` → 어떤 파일이 열려있는지 확인
2. `initialize_hwp()` → MCP용 한글 인스턴스 생성
3. `open_document("C:/...아파트가격동향.hwp")` → 해당 파일 열기
4. `get_text_all()` → 문서 내용 읽기

### 주의사항

| 상황 | 설명 |
|------|------|
| 한글 1개 실행 중 | MCP가 해당 인스턴스에 연결되어 모든 문서 제어 가능 |
| 한글 여러 개 실행 중 | MCP는 하나의 인스턴스에만 연결됨. `list_all_hwp_windows()`로 확인 후 `open_document()`로 필요한 파일 열기 |
| 한글 실행 안 됨 | `initialize_hwp()` 호출 시 자동으로 새 한글 실행 |
